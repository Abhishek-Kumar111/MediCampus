...
exports.login = async(req,res)=>{
    try{
        ...
        if(isExist && await bcryptjs.compare(password,isExist.password)){
            ...
            const token = jwt.sign({ userId: isExist._id }, 'Its_My_Secret_Key');//token banaya and will store it into cooki..
...
exports.sendOtp = async(req,res)=>{ //when click on forgot password in frontend and enter the email than that email we first check than send password 
    try{
...
exports.registerStudentByStaff = async(req,res)=>{
    try{
        ...
        const mailOptions = {
            from: process.env.EMAIL,
            to: body.email,
            subject: 'Password for mediCamp',
            text: `Hi, You password for mediCamp is ${token} whose email id is registered email id ${body.email} `
        };
...
exports.addStaffsByAdmin = async(req,res)=>{
    try{
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Password for mediCamp',
            text: `Hi, You password for mediCamp is ${password} whose email id is registered email id ${email} for Staff Portal`
        };
